"""
Create CR_Score playbook notebooks programmatically.
"""
...
notebook_01 = create_notebook([
    md_cell("""# CR_Score Playbook 01: Quick Start
...
- No PySpark required!"""),
    ...
    md_cell("""## Step 1: Setup and Load Data
...
print("[OK] Scorecard built and scores predicted!")"""),
    ...
    md_cell("""## Step 3: Understand the Results

Let's see what our scorecard produced."""),
    
    code_cell("""# Score statistics
...
print(f"  Std:    {scores.std():.0f}")"""),
    ...
    code_cell("""# Plot score distribution
...
print("Higher scores = Lower risk (good separation!)")"""),
    ...
    md_cell("""## Step 4: Evaluate Performance
...
print(f"  - It can distinguish between good and bad customers!")"""),
    ...
    md_cell("""## Step 5: See Which Features Were Selected

Let's understand what the model is using."""),
    
    code_cell("""# Get pipeline summary
...
for i, feature in enumerate(summary['selected_features'], 1):
    ...
    code_cell("""# See feature importance (IV values)
...
print("  > 0.3: Very Strong")"""),
    ...
    md_cell("""## Step 6: Score New Applications

Now let's use our scorecard to score new loan applications."""),
    
    code_cell("""# Take first 10 applications from test set as "new" applications
...
print(new_applications[display_cols].to_string(index=False))"""),
    ...
    code_cell("""# Make decisions based on scores
...
print(new_applications['decision'].value_counts())"""),
    ...
    md_cell("""## Step 7: Save Your Scorecard

Let's save the scorecard for production use."""),
    
    code_cell("""# Save pipeline
import pickle
...
with open(output_dir / '01_quickstart.ipynb', 'w', encoding='utf-8') as f:
    json.dump(notebook_01, f, indent=1)

print("[OK] Created 01_quickstart.ipynb")
print("\\nRun: jupyter notebook playbooks/01_quickstart.ipynb")
